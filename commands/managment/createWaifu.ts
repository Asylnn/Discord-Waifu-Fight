...
export default async function createWaifu(message: message, user: user, args: Array<string>){
  ...
  if(user.lvl < LEVEL_PERMISSIONS.createWaifu){message.reply(eval(getLoc)("lvl_too_low")); return true;}

  let url = args[1]
  if(url == undefined){message.reply(eval(getLoc)("invalid_url")); return true;}
  ...
  try {
    ...
    message.reply(eval(getLoc)("error_create_custom_waifu"))
  }
}
