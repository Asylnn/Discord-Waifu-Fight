...
export default async function sendterms(message: message, user: user){
  ...
  message.reply(eval(getLoc)("deal_send_terms"))
  ...
  if(!dealUser){message.reply("fatal error (well not fatal but pretty bad) couldn't find your trade partener on the discord cache"); return true}
