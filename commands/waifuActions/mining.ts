...
commandManager?.create({
  name:"mining",
  type:"CHAT_INPUT",
  description:"make a waifu go mine rare minerals",
  options:[
    {
      name:"w",
...
export default async function exp(message: message, user: user, args: Array<string>, interaction: Discord.CommandInteraction){
  ...
  message.addResponse(eval(getLoc)("went_mining"))
  waifu.action = {createdTimestamp:message.createdTimestamp, type:"mining", timeWaiting: waifu.timeWaiting("exploration", lvl), lvl:lvl}
}
